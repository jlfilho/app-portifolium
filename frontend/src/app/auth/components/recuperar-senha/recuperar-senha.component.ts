...
@Component({
  selector: 'acadmanage-recuperar-senha',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class RecuperarSenhaComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Verificar se há código e email na URL
    this.route.queryParams.subscribe(params => {
      this.codeFromUrl = params['code'] || null;
      this.emailFromUrl = params['email'] || null;

      if (this.codeFromUrl && this.emailFromUrl) {
        // Se tem código e email na URL, ir direto para o passo 2
        this.step = 2;
        this.codeSent = true;
        this.initForms();
        // Preencher formulário com dados da URL
        this.resetPasswordForm.patchValue({
          recoveryCode: this.codeFromUrl
        });
  ...
  initForms(): void {
    ...
    this.resetPasswordForm = this.fb.group({
      recoveryCode: [this.codeFromUrl || '', [Validators.required]],
  ...
  redefinirSenha(): void {
    ...
    this.recoveryService.resetPassword(email, recoveryCode, newPassword).subscribe({
      next: (response) => {
        this.isLoading = false;
        const message = response || 'Senha redefinida com sucesso! Redirecionando para o login...';
        this.showMessage(message, 'success');
        
        // Redirecionar para login após 2 segundos
        setTimeout(() => {
          this.router.navigate(['/login'], {
            queryParams: { message: 'Senha redefinida com sucesso! Você já pode fazer login.' }
          });
        }, 2000);
      },
      error: (error) => {
